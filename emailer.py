...
=====================================
Sends a formatted HTML email listing new funding opportunities.
...
<div style="padding:16px;">
  ...
  <p style="font-size:11px;color:#999;">
    Sent by Funding Opportunity Scanner ·
...
def send_alert(opportunities: list[dict]) -> bool:
    ...
    msg["Subject"] = f"[Funding Alert] {count} new opportunit{'y' if count==1 else 'ies'} — {today}"
    msg["From"]    = EMAIL_FROM
...
def _plain_text(opportunities: list[dict]) -> str:
    lines = [f"Funding Opportunity Alert — {datetime.utcnow().strftime('%Y-%m-%d')}\n"]
